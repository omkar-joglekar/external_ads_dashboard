...
#Queries
rows = run_query('''select   CASE   WHEN lead_source = 'SPRINGFACEBOOK' THEN 'FACEBOOK' 
                WHEN lead_source_f is null then lead_source 
                else lead_source_f END AS lead_source2, case when date_f is null then lead_created_date else date_f end as date_2,sum(total_leads),  sum(verifiedleads)
...
if lead_source_filter == "ALL":
    ...
    query_all_lead_sources = '''
                       select case when date_f is null then lead_created_date else date_f end as date_2,sum(total_leads),  sum(verifiedleads)
                   , sum(convertedleads), NULLIF(SUM(convertedleads), 0) / NULLIF(SUM(total_leads), 0)  AS Lead_to_Opp,
                   sum(fundedleads),NULLIF(SUM(fundedleads), 0) / NULLIF(SUM(total_leads), 0)  AS Lead_to_Funded
                   ,NULLIF(SUM(fundedleads), 0) / NULLIF(SUM(convertedleads), 0)  AS Opp_to_Funded,
                   sum(cost),
                   ...
                   NULLIF(SUM(cost), 0) / NULLIF(SUM(convertedleads), 0)  AS CPOpp,
                   NULLIF(SUM(cost), 0) / NULLIF(SUM(fundedleads), 0)  AS CPFunded
                  from CD_ANALYTICS_TESTDB.OMKAR.SPRING_ADS_DASHBOARD where lead_source in 
    ...
    query_all_lead_sources2 = '''select CASE WHEN lead_source = 'SPRINGFACEBOOK' THEN 'FACEBOOK' 
                    WHEN lead_source_f is null then lead_source 
